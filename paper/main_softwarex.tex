%% ============================================================
%% CurricuMap — SoftwareX Original Software Publication
%% Official Elsevier elsarticle template (submission format).
...
\begin{keyword}
Learning analytics \sep Curriculum mapping \sep Competency assessment \sep Data provenance \sep Reproducible research
\end{keyword}

\end{frontmatter}

%% Highlights are submitted separately in SoftwareX; included here for completeness.
...
%% ---- Required Metadata: code metadata table (mandatory) ----
\begin{table}[!ht]
\footnotesize
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{|l|p{3.8cm}|p{7.2cm}|}
\hline
\textbf{Nr.} & \textbf{Code metadata description} & \textbf{Metadata} \\ \hline
C1 & Current code version & v0.1.0 \\ \hline
C2 & Permanent link to code/repository used for this code version & \texttt{https://doi.org/10.5281/zenodo.22140690} \newline (GitHub: \texttt{tkucukdeniz2/curricumap}, tag \texttt{v0.1.0}) \\ \hline
C3 & Permanent link to Reproducible Capsule & n/a \\ \hline
C4 & Legal Code License & MIT \\ \hline
C5 & Code versioning system used & git \\ \hline
C6 & Software code languages, tools, and services used & Python ($\geq$3.10) \\ \hline
C7 & Compilation requirements, operating environments \& dependencies & pandas, numpy, pyyaml, jsonschema, rapidfuzz, openpyxl, jinja2 \\ \hline
C8 & Link to developer documentation/manual & \texttt{README.md} \\ \hline
C9 & Support email for questions & \texttt{tkdeniz@iuc.edu.tr} \\ \hline
\end{tabular}
...
With seed 7 the generator emits 878 grade records across 15 distinct courses, including the retakes and sentinel zeros it injects for realism. The \texttt{run} command reduces these to a $60\times5$ student-by-domain matrix. Classification maps all 15 courses to a domain by keyword rule, with none ambiguous and none left unmapped. Coverage is reported either way, so a run in which a course falls outside the taxonomy surfaces it explicitly rather than dropping it silently. Every per-course decision, with its matched pattern and method, is recorded in \texttt{provenance.csv}, so each cell of the matrix traces back to the rule that produced it.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig1_audit_report.png}
\caption{The self-contained \texttt{audit.html} report from the illustrative run (ELT-YOK taxonomy, seed 7, 60 synthetic students). It pairs the coverage summary (five domains, fifteen mapped courses, none unmapped, none ambiguous) with per-domain descriptives and Cronbach's alpha shown as banded bars.}
\label{fig:audit}
\end{figure}
...
The source code and the five example taxonomies are openly available under the
MIT license (Table~\ref{tab:codemeta}). The exact version described in this paper
is archived at \texttt{https://doi.org/10.5281/zenodo.22140690}.
No personal data are distributed: the
packaged synthetic generator reproduces every example without access to student
records.
